...
invisible(lapply(c("Seurat", "SingleCellExperiment", "scRNAseq", "rappdirs"), requireNamespace, quietly = TRUE))
...
# Define a persistent directory for downloads
persistent_dir <- file.path(rappdirs::user_data_dir("DamageDetective"), "datasets")
dir.create(persistent_dir, showWarnings = FALSE, recursive = TRUE)

# Download files
tar_file <- file.path(persistent_dir, "pbmc_1k_v3_filtered_feature_bc_matrix.tar.gz")
download.file(url, tar_file, mode = "wb", quiet = TRUE)

# Extract files
untar(tar_file, exdir = persistent_dir)
extracted_dir <- file.path(persistent_dir, "filtered_feature_bc_matrix")
