...
This approach was inspired by `DoubletFinder`—a high-performing tool for filtering doublets, another prominent scRNA-seq artifact. Here, artificial profiles of doublets are created by random combination of existing cells. All cells are combined processed before being compared, ultimately finding the proportion of nearest neighbours of artificial origin (pANNs). True cells that show the highest pANN, in accordance to anticipated doublet formation rate, are then labelled as doublets. 

In `DamageDetective`, a score from zero to one is similarily calculated for each cell based on comparisons to artificial profiles where scores closer to one indicate a higher likelihood of damage. This score is used to filter cells, either automatically using the default threshold or manually according to a user-defined threshold.
...
An additional, and fundamental, outcome of loss of membrane integrity is the shift in the library composition. This phenomenon arises because not all RNA in a cell is at the same risk of loss. RNA that is enclosed by additional membranes, such as inside the mitochondria or nuclear speckles, is protected compared to that free in the cytoplasm. Viable cells thus have a high proportion of cytoplasmic RNA compared to damaged cells where RNA has escaped, leading to the proportion of protected RNA appearing elevated. This forms the basis for simulating damage in `DamageDetective`.

Artificial damage profiles are created in `DamageDetective` by simulating cytoplasmic RNA loss relative to a specified damage level between 0 and 1. By default, this lies within the range of 0.65 and 1, drawn from a normal distribution. This is done according to the assumption that the proportion of RNA lost is directly related to the extent of cellular damage, i.e., losing at least 65 % of cytoplasmic RNA must reflect considerably damaged cells that would be considered worthy of excluding from an analysis. Transcripts are sampled from each cell without replacement based on gene abundance until the target loss for the cell is reached.

Simulations are collected for cells belonging to the same cluster of the input data to generate cell-type specific profiles of damage. The idea here is create a pool of damaged cells that represent each unique cell type present in the heterogeneous sample. Depending on the target damage of a cell, the artificial cells retain portions of true gene expression associated with their cell type of origin while exhibiting characteristics of damage, such as reduced complexity, smaller libraries, and higher mitochondrial RNA. This provides a realistic basis for comparison and ultimately a means of estimating how damaged a cell is.


### Estimating true damage by comparison to artificial damage

`DamageDetective` estimates the damage level of true cells by measuring their similarity to artificially damaged cells. Like `DoubletFinder`, this measurement is rooted in a principal component analysis (PCA), though instead of using full gene expression values is done using cell quality control (QC) metrics, including library size, complexity, and mitochondrial RNA proportion- a computationally efficient but still biologically informative alternative. 

Using Euclidean distances between the PCA embeddings of cells, a set of $k$ nearest neighbours is found for each cell via the HNSM (hierarchical navigable small world) algorithm. This is optimized for high dimensional data and is implemented using C++ in R using the `RcppHNSW` package. 

The proportions of a cell's nearest neighbours that originate from each artificial profile (pANN) is calculated. In other words, if there were five clusters in a dataset, there will be five sets of artificial profiles and five pANN scores for each cell. After scaling according to the size of the artificial profile, the maximum pANN is assigned to each cell. This score naturally lies between 0 and 1 and indicates the similarity of true cells to damaged profiles. This is the main output of `DamageDetective` and can be thought of as answer the question, on a scale of 0 to 1, how likely is it that this cell is damaged? 
